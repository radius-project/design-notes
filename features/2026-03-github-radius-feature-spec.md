...
1. The user is taken to the Copilot web interface with a prompt already given to create an application definition.
    ...
    - **Resource composition rules**. For each Resource Type, a set of rules that define how resources can be combined. For example, a web service can connect to a database, but not the reserve.
