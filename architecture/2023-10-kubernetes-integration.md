...
To adopt Radius, we ask users to describe their application using Bicep and Radius' application model. Moving customer workloads to Radius' application model supports our strategy for the project, we're trying to evangelize a broadly-applicable **cloud-native abstraction** and then build multiple implementations of that abstraction. As the end goal: users that adopt Radius can seamlessly move between Kubernetes, on-premises platforms, *and serverless hosting platforms provided by various clouds*. Unfortunately migrating existing applications complicated and dangerous work. To make that worse, few users actually need the kind of hosting platform portability we're planning, and we're a ways off from building it.

We can meet users where they are by 1) replacing less of the user's deployment tooling, 2) providing a quicker time-to-value by being more incremental to adopt. Users will be able to quickly benefit from features like Recipes and Connections without replacing or rewriting existing deployment assets.

While the proposal details a different experience for *users*, there is little proposed change for our architecture and investments in Radius other than documentation and samples.
...
This digaram presents the algorithm as a flowchart:

```mermaid
flowchart TD
  Controller{Has operation in progress?} -->|Yes| OP
  Controller -->|No| DeleteD
  OP{Is operation complete} -->|Yes| DeleteD
  OP -->|No| Requeue
  DeleteD{Is being deleted?} -->|Yes| Delete
  Delete[Start delete operation] -->|Requeue| Requeue
  DeleteD --> |No| UpdateD
  UpdateD{Needs create or update?} -->|Yes| Update
  Update[Start put operation] --> |Requeue| Requeue
  UpdateD --> |No| Ready[Update resource state]
  Requeue -->|Save state| Controller
```
...
The data sent to Radius from Kubernetes looks like the following example:

**HTTP PUT /planes/radius/local/resourceGroups/default-demoapp/providers/Applications.Core/containers/democontainer**
...
```json
{
  ...
  "properties": {
    ...
    "connections": {
      "redis": {
        "source": "/planes/radius/local/resourceGroups/default-demoapp/providers/Applications.Datastores/redisCaches/db"
      }
    },
    "resources": [
      {
        "id": "/planes/kubernetes/local/namespaces/demoapp/providers/apps/Deployment/democontainer"
      }
    ],
...
The Radius controllers will be delivered in their own executable/container which allows us to grant them least-priveledge.
...
- (*Done*) Implement the skeleton (container, executable, helm authoring)
- (*Done*) Add support for `listSecrets` to the generic client
- (*Done*) Add `resourceProvisioning` and `resources` to container type
- (*Done*) Implement Recipe controller and integration tests
- (*In PR*) Implement Deployment controller and integration tests
- (**Required for MVP**) Add E2E test for demo scenario
- (**Required for MVP**) Add Deployment webhook to `pause` deployments
- Add overrides for application and environment
- Add Recipe webhook for resource-type validation
- (**Required for MVP**) Add documentation: new tutorial
