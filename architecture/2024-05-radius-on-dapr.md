...
The alternative is that we build our own "index" feature when inserting or deleting records. This would be used when querying resources at a given scope. 

If we use the resource ID as the state store key, an example scheme might look something like this:

```txt
/planes/radius/local/resourcegroups/my-app/providers/applications.core/containers/frontend -> (data for frontend)
/planes/radius/local/resourcegroups/my-app/providers/applications.core/containers/backend -> (data for backend)
/planes/radius/local/resourcegroups/my-app/:index/applications.core -> (list of container resource names)
/planes/radius/local/resourcegroups -> (list of container resource names)
```

This allows us to implement query support with two roundtrips to the database. First, we fetch the index, then we fetch the resources using the names in the index.
...
The only scenario where a dependency on Dapr impacts users is when:
