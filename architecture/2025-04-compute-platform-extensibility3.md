...
* `kind` can be any string that identifies a platform.
...
* `recipe` will be added, which allows configuring default values on all recipes that run in this environment. Platform-specific required parameters like subscription ID and resource group will be added here and provided to all recipes that run for this environment.
...
resource environment 'Applications.Core/environments@2023-10-01-preview' = {
  ...
  properties: {
    compute: {
+      kind: 'aci'   // Allow any name. This is a descriptive label only.
...
We could consider adding a flag to `rad init` that would identify specific compute platforms and provide the registration of default recipes for that platform, based on a pre-defined set of compute type names. For example, `rad init aci` would install a set of default recipes for the ACI platform.
...
* Environment core type allows multiple environment variables to be set up to be passed so that recipes can run on any platform. For example, the customer may call their environment "Azure".
* How will platform secrets and/or identity be managed in Radius that allow Radius to deploy to non-Kubernetes platforms.
* At deploy time can we validate that all properties required by a recipe have been provided by the environment plus the type?
* Do we need `rad group create`?
