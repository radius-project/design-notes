...
1. Support for `rad resource-type create` and `rad resource-provider create` with interactive experience or from flags

    Creating a resource type from flags may not be a scalable solution when the resource type schema definition is complex. We can explore interactive experience for creating resource types.
    ...
    ```
    This command will prompt the user to enter the resource provider name, resource type name, properties, and version. The user can enter the details and the resource type will be created.
