...
5. Deb makes changes to the resource type definition of `Mycompany.Messaging/plaidServices`
    ...
    rad resource-type create -f plaid.yaml
    ```
    Note that the experience for updates and handling breaking changes and non-breaking changes are detailed in the [authoring experience feature spec]((https://github.com/radius-project/design-notes/pull/74)
...
**Flags**
- `-f`, `--from-file` : The file that contains the resource type definition
...
**Flags**
- `-f`, `--from-file` : The file that contains the resource properties
...
```bash
rad resource-provider create -f plaid.yaml
```
...
1. Support multi-tenancy for resource providers
    ...
    ```bash
    rad resource-provider create -f plaid.yaml --tenant mytenant
    ```
...
1. For deleting a resource type or resource provider, we decided to do the following :
    ...
    2. CLI - Prompt the user with a warning message that there are resources provisioned with the resource type. If the user confirms deletion, API behavior is exercised.
