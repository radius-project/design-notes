...
> There is no new Radius specific user experience for this scenario. Customers are only interacting with Flux or ArgoCD. However, note that we're combining two different but existing user experiences (i.e. GitOps `git push` + Radius `*.bicep`).
